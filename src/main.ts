...
async function loadSettings() {
  try {
    ...
    const editorSettings = await s.get<{ fontSize?: number; tabSize?: number }>(
      "editor",
    );
    if (editorSettings) {
      ...
      if (editorSettings.tabSize)
        editor.updateOptions({ tabSize: editorSettings.tabSize });
    }
...
function updateMemUI(entries: any[]) {
  ...
  const needRebuild = existing.length !== entries.length;
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (card) {
      ...
      if (rows[1])
        rows[1].querySelector("span:last-child")!.textContent = formatBytes(
          e.free,
        );
      if (e.mem_type !== "gc") {
        if (rows[2])
          rows[2].querySelector("span:last-child")!.textContent = formatBytes(
            e.persist,
          );
        if (rows[3])
          rows[3].querySelector("span:last-child")!.textContent = formatBytes(
            e.peak,
          );
      }
    }
...
const protoPollSlider = document.getElementById(
  "proto-poll-slider",
) as HTMLInputElement;
const protoPollValueLabel = document.getElementById("proto-poll-value")!;
protoPollSlider?.addEventListener("input", () => {
  protoPollInterval = parseInt(protoPollSlider.value, 10);
  protoPollValueLabel.textContent = `${protoPollInterval} ms`;
...
function isProtoTabActive(): boolean {
...
setupResize("resize-h", "col", (delta) => {
  ...
  const w = Math.max(
    200,
    Math.min(800, rp.getBoundingClientRect().width / uiScale - delta),
  );
...
let exceptionDecorations: monaco.editor.IEditorDecorationsCollection | null =
  null;
...
const fbSourceSelect = document.getElementById(
  "fb-source",
...
function populateInfoTab(sysinfo: any, version: any, port: string) {
